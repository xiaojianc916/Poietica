...
Creative work rarely begins with a polished plan. It starts with an unfinished sentence, a question, a reference, a feeling, or a direction that has not become clear yet.

Poietica brings those fragments into one place: a conversation. You describe where you are stuck, the agent works with tools, skills and MCP servers, and every step stays inspectable and reversible.
...
Runs are local-first. The event log is written before anything is rendered, so an interrupted run can always resume.
...
Poietica is designed around predictable local behavior:
...
> **Active development**  
> The current focus is the reliable agent foundation: typed IPC, encrypted local run persistence, tool and MCP execution, and bounded AI workflows.
...
- **Tauri + Rust** — desktop integration, encrypted local state, system capabilities, and security boundaries.
...
| `pnpm build:release` | Build and sign the Windows installer and updater artifacts (NSIS). |
...
> Session updates are persisted before they are rendered, so an interrupted run can always be replayed.
