 ...
 *
 * 用途只有一个：一次性导入。目录的原料是 provider list 的快照（模型 id、上下文、
 * capabilities、effort 全在里面），写入仍走官方的 catalog add —— 协议、接口地址、
 * 模型清单由这份文档供，密钥由原生侧从全局 config.toml 取，全程不进渲染层。
 *
 * 与 builtin-provider-catalog 的 agentProviderCatalogDocument 是两种产地：那一份
 * 回答「内置名单上这家有什么」，这一份回答「你自己配置里这家有什么」。形状相同，
 * 写入管线是同一条。
 *
 * 没有正整数上下文的模型被跳过：对方的 catalogModelToCapability 会把它们整条丢掉，
 * 与其让它静默消失，不如在这里就跳过并说明。密钥永不进这份文档。
 */
export function agentProviderImportDocument(provider: AgentProviderState): string {
  ...
  for (const model of provider.models) {
    ...
    const id = model.alias.startsWith(`${provider.id}/`)
      ? model.alias.slice(provider.id.length + 1)
      : model.alias
