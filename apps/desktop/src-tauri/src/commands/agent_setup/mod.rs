...
//! 那个外部 CLI 装没装。`super::agent` 是一条活着的会话，状态在内存里，寿命
//! 是一次连接。所以两边不是同一个 `config`，也不该合成一个。
//!
//! 依赖单向：`super::agent::runtime` 起连接之前向 `profile::launch_env` 要环境
//! 变量；这一侧不认识会话，也不得反向引用 `super::agent`。
//!
//! 分四个文件而不是一个，因为其中两条是各自封闭的白名单入口：`cli` 只放行
